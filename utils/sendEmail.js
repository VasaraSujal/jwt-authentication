...
const sendVerificationEmail = async (userEmail, otp, username) => {
  ...
  if (!myEmail || !clientId || !clientSecret || !redirectUrl || !refreshToken) {
    console.log('Email credentials missing. Skipping email sending but continuing registration process.');
    // Return without sending email but don't throw an error
    // This allows registration to continue even without email verification
    return;
  }
